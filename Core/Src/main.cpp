...
#include "../Test/Inc/test_basic.hpp"
#include "../Test/Inc/test_graphical_ui.hpp"
#endif
...
/* Definitions for SysLoop */
osThreadId_t SysLoopHandle;
const osThreadAttr_t SysLoop_attributes = {
  .name = "SysLoop",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityHigh,
};
/* Definitions for KeypadMutex */
osMutexId_t KeypadMutexHandle;
const osMutexAttr_t KeypadMutex_attributes = {
  .name = "KeypadMutex"
};
...
/* Definitions for GUIStateMutex */
osMutexId_t GUIStateMutexHandle;
const osMutexAttr_t GUIStateMutex_attributes = {
  .name = "GUIStateMutex"
};
...
void StartSysLoopTask(void *argument);
...
int main(void)
{
  ...
  // set up all of the structures required
  guiStateInit(GUIStateMutexHandle);
  spi_init(SPIMutexHandle);
  // AT132 setup
  at132.init(&hspi1, RNGMutexHandle);
  logic_main_set_rng(&at132);
  logic_main_set_keystore(&at132);
  // set I2C1 - ideally remove this
  i2c1_init(&hi2c1);
  ...
  osKernelInitialize();
  /* Create the mutex(es) */
  /* creation of KeypadMutex */
  KeypadMutexHandle = osMutexNew(&KeypadMutex_attributes);
  ...
  /* Create the thread(s) */
  /* creation of USBMain */
  USBMainHandle = osThreadNew(StartUSBTask, NULL, &USBMain_attributes);
  ...
  /* creation of SysLoop */
  SysLoopHandle = osThreadNew(StartSysLoopTask, NULL, &SysLoop_attributes);
...
static void MX_I2C2_Init(void)
{
  ...
  hi2c2.Instance = I2C2;
  hi2c2.Init.Timing = 0x00602173; // 400khz = 0x00602173, 100khz = 0x10707DBC;
...
void StartUSBTask(void *argument)
{
  ...
  {
#ifdef LIVE_UNIT_TESTING
	test_basic_flash_led();
	test_display_showAllScreens();
#endif
    osDelay(100);
  }
  /* USER CODE END 5 */
...
void StartSPITask(void *argument)
{
  /* USER CODE BEGIN StartSPITask */
	//SPIMutexHandle = (osMutexId_t) SPIMutexHandle;
	/* Infinite loop */

  for(;;)
  {
#ifndef LIVE_UNIT_TESTING
	  at132.rtos_rng_update();
	  at132.rtos_keystore_update();
#endif
    osDelay(1);
  }
...
/* USER CODE BEGIN Header_StartSysLoopTask */
/**
* @brief Function implementing the SysLoop thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartSysLoopTask */
void StartSysLoopTask(void *argument)
{
  /* USER CODE BEGIN StartSysLoopTask */
  /* Infinite loop */
  for(;;)
  {
#ifndef LIVE_UNIT_TESTING
  ...
  /* USER CODE END StartSysLoopTask */
}
