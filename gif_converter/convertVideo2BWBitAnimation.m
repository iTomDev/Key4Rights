...
clc
clear
vid = VideoReader('vids/helix.mp4');
vid.CurrentTime = 0.5;
...
% create image array
fmax = round((vid.NumFrames / skipcount),0);
bwimgs = zeros(63,107,fmax);
...
while hasFrame(vid)
    if( i < skipcount)
        ...
        %binarize
        bwframe = imbinarize(imresize(   imcrop(vidFrame, [180 90 640 360])  ,0.23), 'adaptive'); 
        bwimgs(:,:,frameidx) = bwframe(:,:,1);
        frameidx = frameidx + 1;
        
        imshow( imresize(uint8(bwframe(:,:,1)*255),(1/0.23)) )
